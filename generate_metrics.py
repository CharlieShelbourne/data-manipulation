...
def generate_metrics(
    ...
    metrics_df = getattr(
        df.groupby(by=metrics_cofigs[0]["grouping"]),
         "count"
        )()[[]].reset_index()
    ...
    for params in metrics_cofigs:
        ...
        grouped_apply = getattr(
            df.groupby(by=params["grouping"])[params["column"]], 
            "apply"
            )
...
def generate_cumsum_metrics(
    ...
    for params in metrics_cofigs:
        grouped_apply = getattr(
            df.groupby(params["grouping"]).expanding()[params["column"]], 
            "apply"
            )
